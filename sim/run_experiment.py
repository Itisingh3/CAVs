...
def _round_outcome(rng: random.Random, selected_ids: set[str], conditions: dict[str, NodeCondition], features: dict[str, NodeFeatures], f: int, density: int, congestion: float) -> dict:
    ...
    return {"agreed": agreed, "latency_ms": 20 + density * .35 + mean_rtt * 45 + len(selected_ids) * 1.8 + malicious_fraction * 90 + (0 if committed else 120), "throughput_packets": max(0.0, (len(agreed) if committed else 0) * (1 - congestion * .30)), "pdr": max(0.0, min(1.0, reliability * (1 - congestion * .22) * (1 - malicious_fraction * .35))), "control_bytes": len(selected_ids) * 3309 * 3, "committed": float(committed), "false_removals": float(sum(1 for node, state in conditions.items() if not state.malicious and node not in selected_ids)), "malicious_selected": float(sum(1 for node in selected_ids if conditions[node].malicious))}
...
def run(variant: str, seed: int, density: int, output: Path, rounds: int = 300, warmup_rounds: int = 60) -> Path:
    ...
    for index in range(rounds):
        congestion = min(.95, max(.05, density / 350 + .23 * (1 + math.sin(index / 17)) / 2 + rng.uniform(-.08, .08)))
        features = {node: _features(rng, conditions[node], congestion) for node in node_ids}
        outcome = _round_outcome(rng, engine.consensus_ids(), conditions, features, f, density, congestion)
    ...
    manifest = {"variant":variant,"seed":seed,"density":density,"rounds":rounds,"warmup_rounds":warmup_rounds,"engine":"controlled-development-model-not-veins","paired_seed":seed,"python":sys.version,"platform":platform.platform(),"warning":"Development model only. Do not cite these as SUMO/Veins/OMNeT++ or real-network results."}

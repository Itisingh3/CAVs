...
def main() -> None:
    ...
    parser.add_argument("--ml-predictor", choices=tuple(PREDICTOR_FACTORIES), default="logistic_regression", help="model selected before this untouched-test run")
    ...
    study_manifest = {
        ...
        "ml_predictor": arguments.ml_predictor,
    ...
    for density in arguments.densities:
        for seed in split.test:
            for variant in ("baseline", "ml"):
                print(run(variant, seed, density, arguments.output, arguments.rounds, arguments.warmup_rounds, arguments.ml_predictor))
